...
class StressResults(Resource):
    ...
    def get(self):
        ...
        file_url = urljoin(base_url.replace('/stressResults', ''), f'download/{secure_filename("DS2.xlsx")}')
...
class AllowanceResults(Resource):
    ...
    def get(self):
        ...
        base_url = request.base_url  # Get the base URL of the request
        # Construct full URL for the file
        file_url = urljoin(base_url.replace('/allowanceResults', ''), f'download/{secure_filename("DS1.xlsx")}')
...
def download_file(filename):
    ...
    if filename == "DS1.xlsx":
         file_path = DS1_PATH
    elif filename == "DS2.xlsx":
        file_path = DS2_PATH
    else:
